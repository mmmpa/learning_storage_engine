...
[./src/most_simple.rs](./src/most_simple.rs)

## set

value を特定するための key とセットでレコードを set する。log の原則に従い、レコード上に同 key があるかないかに関わらず、set のたびにレコードを追加する。
...
[./src/hash_index.rs](./src/hash_index.rs)
...
分割が行われても log 全体のサイズは際限なく膨らみ続ける。どのような用法であれ、log の原則からいずれはディスクをあふれることになる。

そこでさらに一定のサイズを超えた段階で compaction が行われる。

### 重複するレコードを排除する

log の原則から言うと既にある log を編集することはないので、正確には膨れた log から現在有効なレコードを取りだし、重複のない log を作成し、以後はその新しい log を参照するようにする。
...
[./src/log_structured_merged_tree.rs](./src/log_structured_merged_tree.rs)
...
ソート状態を保たなければならないため、log の原則上 C1 には直接書き込めない。そこでまず C0 にデータを挿入する。(通例、赤黒木などの平衡木が用いられる)

一定のデータが溜まった段階で古い C1 と C0 の間で compaction/merging 行う。双方ともに順序付きであるから merge sort の原則で効率的に行われる。
...
特に key の量が限られる場合はシンプルな実装が読み書きともにパワフルであることがわかった。

また書き込み速度を保ちつつ、読み出し時の速度を許容範囲に収めるための LSM-Tree は応用が効きそうなデータ構造だとわかった。

## LSM-Tree と B-Tree

今回自分で書いた LSM-Tree は二階層だが、実際はデータを扱うためにどんどん深くなるのでチェック回数はデータ量により膨れる。一方で B-Tree はおおむね数回に収まるため read はだいたい早い。

## 分散データベースへ

LSM-Tree の SSTable と index の組み合わせは分散型データベースの設定を考える際にも役立つ。

例えば sorted key による分散を行えば範囲クエリに強いなどの利点を享受できる。一方で分散の偏りを招く可能性がある。

hash key による分散では効率的な負荷分散が行われるが、範囲クエリは分散先を逐次計算で導く必要があり効率的とは言えない。
