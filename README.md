...
segment file のレコードを key の順序にして保存する SSTable というフォーマットがある。(重複する key は無い)

ソート済みのレコードは範囲クエリに対応する他、Hash index 保持する key が部分的でかまわないという利点がある。

1 と 11 と 21 の開始位置を知ればその間の id がどこにあるかは十分限定される上、よく知られているようにソート済みデータからの検索効率は高い。

# Log-Structured Merge-Tree (LSM-Tree)

ソート状態を保つために順序付き木とソート済み segment file を使うLSM-Tree という仕組みがある。

## ソート状態を保つために

ソート状態を保たなければならないため、log の原理上 segment file には直接書き込めない。そこでまず順序付き木にデータを挿入する。通例赤黒木などの平衡木が用いられる。

ノードが一定の数に達すると木はデータを segment file に逃し、もし segment file があれば compaction/merging を行い必要に応じて再分割する (merge sort の原理で compaction は効率的に行える)。
